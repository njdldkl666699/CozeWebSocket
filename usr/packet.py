class EventType:
    ...
    CONVERSATION_CHAT_IN_PROGRESS = "conversation.chat.in_progress"  # 对话进行中
    CONVERSATION_CHAT_IN_PROGRESS_LEGACY = "conversation.chat.in.progress"
    CONVERSATION_CHAT_COMPLETED = "conversation.chat.completed"  # 对话已完成
    CONVERSATION_CHAT_FAILED = "conversation.chat.failed"  # 对话失败
    CONVERSATION_CHAT_CANCELED = "conversation.chat.canceled"  # 对话已取消
    CONVERSATION_CHAT_CANCELLED = CONVERSATION_CHAT_CANCELED
    ...
    CONVERSATION_AUDIO_TRANSCRIPT_UPDATE = (
        "conversation.audio_transcript.update"  # 用户语音识别实时字幕更新
    )
    CONVERSATION_AUDIO_TRANSCRIPT_COMPLETED = (
        "conversation.audio_transcript.completed"  # 用户语音识别完成
    )
    ...
    "data": {
        ...
        "output_audio": {
            ...
            "pcm_config": {
                "sample_rate": 8000,
                "frame_size_ms": 100,
                "limit_config": {"period": 1, "max_frame_num": 50},
            },
            "speech_rate": 0,
        },
        "turn_detection": {
            "type": "server_vad",
            "interrupt_config": {"mode": "keyword_contains", "keywords": ["闭嘴", "你好扣子"]},
        },
        "asr_config": {"enable_ddc": True, "hot_words": ["闭嘴", "你好扣子"]},
        "event_subscriptions": [
            "error",
...
append = {
    ...
    "data": {"delta": "base64EncodedAudioDelta"},
}

cancel = {"id": "event_id_123459", "event_type": "conversation.chat.cancel"}

disconnected = {"event_type": "client.disconnected"}


def build_chat_update() -> dict:
    return update


def build_audio_append(delta: str) -> dict:
    return {
        "id": "event_id_123458",
        "event_type": "input_audio_buffer.append",
        "data": {"delta": delta},
...
def build_cancel() -> dict:
    return {"id": "event_id_123459", "event_type": "conversation.chat.cancel"}
